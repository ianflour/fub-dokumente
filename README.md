...
| Ordner | Inhalt |
|---|---|
| `scripts/` | alle Werkzeuge. `menue.py` ist das Menü; `_pfade.py`, `_teilnehmende.py`, `_saal_listen.py` sind geteilte Hilfsmodule (nicht direkt aufrufen). |
| `data/` | alles von Hand Gepflegte: `teilnehmende.txt`, `instagram.csv`, `design.json`, die editierbaren Datenbasen `schichtplan_daten.py` / `gaesteliste_daten.py` / `workshop_daten.py`, die Ablageordner `werkmeldungen/` + `werkmeldung_alt/`, das Nachschlagewerk `saal-referenz/`, Beispieldateien in `beispiele/`. Übersicht: [data/README.md](data/README.md), Erklärung der `design.json`: [data/README_design.md](data/README_design.md). |
| `assets/` | `schriften/` — hier legt die Gestaltung Regular- und Bold-Schnitt ab (wird automatisch erkannt). |
| `output/` | **alles Erzeugte.** Master, Werkmeldung-Vorlage, alle PDFs, `ausgabe/`, `qr-codes/`, Backups. Jederzeit neu baubar — nichts hier von Hand ändern. |
...
> **Hinweis Master/Vorlage:** `output/Werkdaten_MASTER.xlsx` liegt in
...
```
  ...
  8  Master-Tabelle neu       output/Werkdaten_MASTER.xlsx
  9  Werkmeldung-Vorlage neu  output/Werkmeldung_VORLAGE.xlsx
 10  QR-Codes                 output/qr-codes/ aus data/instagram.csv
 11  Werkmeldung migrieren    data/werkmeldung_alt/ → neue Vorlage
 12  Beispiel-Werkmeldung     data/beispiele/Werkmeldung_BEISPIEL.xlsx (zum Testen)
```

Direkt ohne Menü: `./.venv/bin/python scripts/menue.py 2`
(Argumente durchreichen: `… scripts/menue.py 3 -- --probe`).
